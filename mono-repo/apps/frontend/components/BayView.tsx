...
function BayCard({
  ...
  return (
    <div className="rounded-lg border border-border p-3 flex flex-col gap-2 bg-card hover:bg-muted/20 transition">
      {/* Header */}
      <div className="flex items-center justify-between text-xs">
        <span className="text-muted">Bay</span>
        <span className="font-semibold text-foreground">{String(bay)}</span>
      </div>
      ...
      {train ? (
        <>
          ...
          <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs w-fit", statusColor)}>
...
type Props = {
  trainsets?: Trainset[];
  layout?: { rows: number; cols: number } | "2x8" | "4x4" | number;
...
export default function BayView({
  ...
  const bayList = useMemo(() => {
    let total =
      layout === "2x8"
        ...
        ? layout
        : (layout as any)?.rows * (layout as any)?.cols || 16;
  ...
  const cols =
    layout === "2x8"
      ...
      : (layout as any)?.cols || 8;

  /* Summary stats */
  const filled = bayMap.size;
  const total = bayList.length;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-muted">Total Bays: {total}</span>
        <span className="text-emerald-500">Filled: {filled}</span>
        <span className="text-muted">Empty: {empty}</span>
      </div>

      {/* Grid */}
      <div className={cn("grid gap-2", `grid-cols-${Math.min(cols, 8)} sm:grid-cols-${cols}`)}>
        {bayList.map((bay) => (
